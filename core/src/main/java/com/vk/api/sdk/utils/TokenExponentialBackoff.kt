 ...
 * - TokenPrefStore: хранит токены в SharedPreference
 * - TokenMemStore: хранит токены в HashMap
 */
class TokenExponentialBackoff(
    ...
    class TokenPrefStore(context: Context) : TokenStore {
        ...
        override fun reset(token: String) {
            prefStorage.edit()
                .remove(token)
                .remove(COUNT_PREFIX + token)
                .apply()
        }

        @Synchronized
        override fun restore(token: String, default: Long): Pair<Long, Int> {
            return prefStorage.getLong(token, default) to
                    prefStorage.getInt(COUNT_PREFIX + token, 0)
        }

        @Synchronized
        override fun store(token: String, value: Long) {
            val count = prefStorage.getInt(COUNT_PREFIX + token, -1).inc()
            prefStorage.edit()
                .putLong(token, value)
                .putInt(COUNT_PREFIX + token, count)
                .apply()
        }
