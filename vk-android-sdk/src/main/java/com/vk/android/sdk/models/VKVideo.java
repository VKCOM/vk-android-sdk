...
public class VKVideo extends VKAttachments.VKAttachment implements Parcelable, Identifiable {
     ...
     */
    public VKVideo parse(JSONObject from) {
        id = from.optInt("id");
        ...
        if(!TextUtils.isEmpty(photo_320)) {
            photo.add(VKPhotoSize.create(photo_320, 320));
        }

        photo_640 = from.optString("photo_640");
        if(!TextUtils.isEmpty(photo_640)) {
            photo.add(VKPhotoSize.create(photo_640, 640));
        }
        return this;
    }
    ...
    public static Creator<VKVideo> CREATOR = new Creator<VKVideo>() {
        public VKVideo createFromParcel(Parcel source) {
            return new VKVideo(source);
        }

        public VKVideo[] newArray(int size) {
            return new VKVideo[size];
        }
    };
}
