...
import static com.vk.android.sdk.models.ParseUtils.parseInt;
import static com.vk.android.sdk.models.VKAttachments.*;

/**
 * Describes a photo object from VK.
 */
public class VKPhoto extends VKAttachments.VKAttachment implements Parcelable, Identifiable {
     ...
     */
    public VKPhoto parse(JSONObject from) {
        ...
        if(photo_sizes != null) {
            ...
            if(!TextUtils.isEmpty(photo_75)) {
                src.add(VKPhotoSize.create(photo_75, VKPhotoSize.S, width, height));
            }
            if(!TextUtils.isEmpty(photo_130)) {
                src.add(VKPhotoSize.create(photo_130, VKPhotoSize.M, width, height));
            }
            if(!TextUtils.isEmpty(photo_604)) {
                src.add(VKPhotoSize.create(photo_604, VKPhotoSize.X, width, height));
            }
            if(!TextUtils.isEmpty(photo_807)) {
                src.add(VKPhotoSize.create(photo_807, VKPhotoSize.Y, width, height));
            }
            if(!TextUtils.isEmpty(photo_1280)) {
                src.add(VKPhotoSize.create(photo_1280, VKPhotoSize.Z, width, height));
            }
            if(!TextUtils.isEmpty(photo_2560)) {
                src.add(VKPhotoSize.create(photo_2560, VKPhotoSize.W, width, height));
            }
            src.sort();
        }
        return this;
    ...
    public static Creator<VKPhoto> CREATOR = new Creator<VKPhoto>() {
        public VKPhoto createFromParcel(Parcel source) {
            return new VKPhoto(source);
        }
